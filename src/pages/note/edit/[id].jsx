...
import { ArrowLeft, Save, Clock, Tag, X, Settings, Info, Plus } from 'lucide-react';
...
export default function EditNote() {
    ...
    // Focus title input only on initial component mount
    useEffect(() => {
        if (titleInputRef.current && !note) {
    ...
    useEffect(() => {
        ...
        if (currentNote) {
            setNote(currentNote);
            setLastSaved(new Date(currentNote.updatedAt || currentNote.createdAt));
        }
    }, [id, notes]);
    ...
    useEffect(() => {
        if (note) {
            ...
            if (changesDetected && autoSaveEnabled) {
    ...
    const handleSave = async (isAutoSave = false) => {
        ...
        try {
            const now = new Date();
            await updateNote(id, { ...note, updatedAt: now.toISOString() });
            setLastSaved(now);
    ...
    const handleCreateTag = async (action, data) => {
        if (action === 'create' && data?.name) {
            try {
                ...
                setIsCreateTagOpen(false); // Close CreateTagDialog, return to TagSelectionDialog
            } catch (error) {
        ...
        <div className="min-h-screen bg-bg-primary flex items-center justify-center px-4">
            <Card className="w-full max-w-md">
                <CardContent className="p-6 flex flex-col items-center">
                    <div className="animate-pulse h-6 w-24 bg-gray-300 rounded-md mb-4"></div>
                    <div className="text-center text-text-primary/60">Loading note...</div>
                </CardContent>
            </Card>
        </div>
    );
                                ...
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <Button
                                            ...
                                            size="sm"
                                            className={`flex items-center gap-2 ${!hasChanges ? 'opacity-50' : 'bg-primary hover:bg-primary/90'} transition-all`}
                                        >
