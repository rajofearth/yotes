...
export default function NoteEditor() {
  ...
  const { createNote, updateNote, notes, tags, createTag, isLoading: isNotesLoading } = useNotes(); // Include isLoading
  ...
  const [isTagManagementOpen, setIsTagManagementOpen] = useState(false); //keep to use later.
  const [lastSaved, setLastSaved] = useState(null);
  const [autoSaveEnabled, setAutoSaveEnabled] = useState(true); //keep to use later
  const autoSaveTimerRef = useRef(null); //keep to use later
  ...
  useEffect(() => {
    ...
    if (hasChanges && autoSaveEnabled && !lastSaved) {
...
}, [note, autoSaveEnabled, lastSaved, notes, isCreate, noteId]);
    ...
    try {
      ...
      if(isCreate){
        ...
        if (!isAutoSave) {
            ...
            setNote(prev => ({ ...prev, id: createdNote.id })); // Keep the ID for further edits.
        }
      }else{
