...
export default function SectionView() {
    ...
    return (
        <div className="min-h-screen bg-bg-primary">
            ...
            <main className="max-w-[1920px] mx-auto px-4 py-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2">
                {notes.map(note => (
                    <div key={note.id} className="w-[280px]">
                            <NoteCard note={note} refreshNotes={() => navigate('/', { state: { refresh: true } })} />
                        </div>
                ))}
            </main>
        </div>
